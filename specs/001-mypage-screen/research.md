...
| 알림 상태(FR-008~011e) | `PUT /members/me/notification {enabled}` 존재, `GET /members/me/notification` 존재(2026-09-23 배포 확인, 기본 true), `PUT /members/me/fcm-token` 존재 | 사용자 토글 값을 서버에 반영(실패 시 되돌림), 진입 시 GET으로 로컬을 서버 값에 맞춤 → [contracts/member-notification-api.md](contracts/member-notification-api.md). TBD-06 해소. FCM 토큰 등록은 앱에 FCM 미도입이라 범위 밖 |
...
- **Decision (2026-09-23 추가, 조회 API 배포)**: 서버가 `GET /members/me/notification`(`Member/getNotificationSetting`, 한 번도 바꾸지 않은 회원도 `true`)을 배포해 서버 값을 기준으로 삼는다. `FetchNotificationSettingUseCase`(인증 보장 → `MemberRepository.getNotificationSetting()`, 401은 forceRefresh 후 1회 재시도 → 성공 시 `AppSettingsRepository.setNotificationEnabled(응답 enabled)`, `CancellationException` 외 예외는 삼킴)를 `MyPageViewModel` init에서 1회 호출한다. `ToggleNotificationEnabled`는 진행 중인 조회 Job을 취소해 뒤늦은 응답이 사용자 의사를 덮지 않게 한다. ON_RESUME·권한 변화·앱 초기화에서는 재조회하지 않는다. 로컬 `notification_enabled`는 조회 전·실패 시 표시용 캐시로 유지한다.
- **Rationale**: 요구사항 재정의(2026-09-23)로 토글이 사용자 설정이 됐다. 서버 값이 기준이므로 진입 시 한 번 읽어 로컬을 맞추고, 변경은 서버 반영 성공 후에만 저장해 로컬과 서버가 어긋나지 않게 한다. 조회는 진입 시 1회면 충분하고(서버 문서도 '토글 초기 상태 세팅 용도'), 실패해도 로컬 캐시로 화면이 동작하므로 토스트를 띄우지 않는다. 같은 기기 재설치는 `ANDROID_ID`·`identifierForVendor` 기반 로그인이라 같은 회원이므로 로컬 기본값과 서버 값이 실제로 어긋날 수 있어 진입 조회가 필요하다. 기기 권한은 OS가 푸시 표시를 막으므로 서버에 알릴 필요가 없고, 권한 변화로 사용자 의사를 덮어쓰지 않는다.
- **Alternatives considered**: (a) 로컬 값만 진실로 삼고 서버는 쓰기 전용 — 조회 API 배포 전 방식(v0.3.0). 재설치 시 로컬 기본값 true와 서버 값이 어긋나 폐기. (a') 조회 중 토글을 disabled로 두기 — 디자인에 로딩 상태가 없고 기본값이 양쪽 true라 깜빡임이 드물어, 로컬 캐시 표시 + 조작 시 조회 취소로 대체. (a'') ON_RESUME마다 재조회 — 기기 설정 복귀는 권한만 바뀌므로 불필요한 호출, 진입 1회로 제한. (b) 로컬 먼저 저장하고 서버는 best-effort — 오프라인에서 서버와 영구히 어긋나 푸시 도입 시 사용자 의사와 다르게 동작. (c) 권한이 꺼져도 토글을 enabled로 두고 탭 시 설정 이동 — 요구사항(권한 꺼짐 시 disabled)과 충돌. (d) core 모듈에 두기 — 사용처가 마이페이지뿐이라 feature/map에 둔다(공용화 필요 시 core:ui로 승격).
