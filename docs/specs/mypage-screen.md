...
기존 추적 이력을 유지하는 기능 ID다. 동작은 위 v0.2 정책을 따르며, `BUG_REPORT_OPEN`은 별도 신고 화면이 아니라 통합 의견 시트의 오류·버그 유형을 가리킨다. 아래 이미지 참조는 이전 디자인 근거이며 현재 화면의 시각 검증 기준은 §2와 `ui-plan.md`다.
