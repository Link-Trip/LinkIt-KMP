...
| 알림설정 | 알림 (토글) | 권한 꺼짐: 토글 disabled·off, 행 탭 시 기기 설정 이동. 권한 켜짐: 토글 enabled, 앱 알림 수신 설정에 따라 on/off, 탭 시 수신 설정 전환 + `PUT /members/me/notification` |
...
| 알림 2계층 | (1) 기기 알림 권한: OS가 결정, 앱은 진입·복귀 시 조회만. (2) 앱 알림 수신 설정: 사용자가 토글로 정하는 값, DataStore `notification_enabled`(기본 true)에 저장. 토글은 권한이 켜져 있을 때만 enabled이고 수신 설정을 표시한다. 권한이 꺼져 있거나 조회 불가면 disabled·off |
| 알림 서버 동기화 | 사용자가 토글을 바꿀 때만 `PUT /members/me/notification {enabled}`. 낙관적으로 토글을 먼저 바꾸고, 성공 시 로컬 저장, 실패 시 토글 되돌림 + `알림 설정 변경에 실패했습니다. 다시 시도해주세요.` 토스트. 기기 권한 변화는 서버에 보내지 않는다. 서버 조회 API가 없어 로컬 값이 표시의 진실(`TBD-06`) |
...
| 데이터 | `AppSettingsLocalDataSource`(DataStore, `map_display_type`·`notification_prompted`·`notification_enabled`), `MemberApi`, `FeedbackApi`, `AppInfoProvider`(플랫폼 그래프 제공). 온보딩 완료 키는 002에서 `OnboardingLocalDataSource`로 이관 |
...
| `TBD-06` | 알림 수신 설정 서버 조회 | 서버에 `PUT /members/me/notification`만 있고 조회 API가 없다. 재설치·기기 변경·초기화 후 재로그인 시 로컬 기본값(true)과 서버값이 어긋날 수 있다. `GET /members/me/notification` 또는 로그인 응답에 `notificationEnabled` 포함을 서버에 요청하고, 배포되면 진입 시 로컬을 서버값으로 덮어쓴다 |
